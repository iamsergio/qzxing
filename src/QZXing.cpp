...
QZXing::QZXing(QObject *parent)
    : QObject(parent), tryHarder_(false), lastDecodeOperationSucceded_(false) {
  decoder = new MultiFormatReader();
  setDecoder(DecoderFormat_QR_CODE | DecoderFormat_DATA_MATRIX |
             DecoderFormat_UPC_E | DecoderFormat_UPC_A |
             DecoderFormat_UPC_EAN_EXTENSION | DecoderFormat_RSS_14 |
             DecoderFormat_RSS_EXPANDED | DecoderFormat_PDF_417 |
             DecoderFormat_MAXICODE | DecoderFormat_EAN_8 |
             DecoderFormat_EAN_13 | DecoderFormat_CODE_128 |
             DecoderFormat_CODE_93 | DecoderFormat_CODE_39 |
             DecoderFormat_CODABAR | DecoderFormat_ITF | DecoderFormat_Aztec);
...
QZXing::QZXing(QZXing::DecoderFormat decodeHints, QObject *parent)
    : QObject(parent), lastDecodeOperationSucceded_(false) {
  decoder = new MultiFormatReader();
  imageHandler = new ImageHandler();
...
void QZXing::setTryHarder(bool tryHarder) { tryHarder_ = tryHarder; }

bool QZXing::getTryHarder() { return tryHarder_; }

void QZXing::setTryHarderBehaviour(
    QZXing::TryHarderBehaviourType tryHarderBehaviour) {
...
QString QZXing::foundedFormat() const { return decodedFormat; }
 ...
 *
 * To be able display tag rectangle regardless of the size of the bit matrix
 * rect is in related coordinates [0; 1]. \param resultPoints \param bitMatrix
 * \return
 */
QRectF getTagRect(const QSharedPointer<std::vector<QSharedPointer<ResultPoint>>>
                      &resultPoints,
                  const QSharedPointer<BitMatrix> &bitMatrix) {
  ...
  if (resultPoints->size() == 2) {
    ...
    std::vector<QSharedPointer<ResultPoint>> resultRectPoints =
        detector.detect();
    ...
    return QRectF(QPointF(xMin / matrixWidth, yMax / matrixHeight),
                  QPointF(xMax / matrixWidth, yMin / matrixHeight));
  }
...
QString QZXing::decodeImage(const QImage &image, int maxWidth, int maxHeight,
  ...
  if ((maxWidth > 0) || (maxHeight > 0))
    ciw = CameraImageWrapper::Factory(image, maxWidth, maxHeight,
  ...
  // qDebug() << "Iterations: "<< numberOfIterations << ", sourceFilter: " <<
  // imageSourceFilter;

  for (size_t i = 0; i < numberOfIterations; ++i) {
    try {
      if ((numberOfIterations == 1 &&
           (imageSourceFilter & SourceFilter_ImageInverted)) ||
          i == 1) {
        // qDebug() << "Selecting Inverted Luminance source";
        imageRef = QSharedPointer<LuminanceSource>(
            (LuminanceSource *)(new InvertedLuminanceSource(imageRefOriginal)));
      }
      binz = QSharedPointer<GlobalHistogramBinarizer>(
      ...
      if (!lastDecodeOperationSucceded_ && tryHarder_ &&
          (tryHarderType & TryHarderBehaviour_ThoroughScanning)) {
      ...
      if (!lastDecodeOperationSucceded_ && tryHarder_ &&
          (tryHarderType & TryHarderBehaviour_Rotate) &&
          bb->isRotateSupported()) {
        ...
        for (int i = 0; (i < 3 && !lastDecodeOperationSucceded_); i++) {
          QSharedPointer<BinaryBitmap> rotatedImage(
              bbTmp->rotateCounterClockwise());
          bbTmp = rotatedImage;
  ...
  if (lastDecodeOperationSucceded_) {
    ...
    if (!string.isEmpty() && (string.length() > 0)) {
      ...
      if (!charSet_.isEmpty()) {
        ...
        if (decoder.isValid()) {
          string =
              decoder.decode(QByteArray(res->getText()->getText().c_str()));
        }
#endif
      }
      ...
      try {
        const QRectF rect =
            getTagRect(res->getResultPoints(), binz->getBlackMatrix());
...
QString QZXing::decodeImageFromFile(const QString &imageFilePath, int maxWidth,
                                    int maxHeight, bool smoothTransformation) {
  // used to have a check if this image exists
  // but was removed because if the image file path doesn't point to a valid
  // image then the QImage::isNull will return true and the decoding will fail
  // eitherway.
...
QString QZXing::decodeSubImageQML(QObject *item, const int offsetX,
...
QString QZXing::decodeSubImageQML(const QUrl &imageUrl, const int offsetX,
                                  const int offsetY, const int width,
                                  const int height) {
#ifdef QZXING_QML
  ...
  if (imageUrl.scheme() == "image") {
    ...
    if (!fileInfo.exists()) {
      qDebug() << "[decodeSubImageQML()] The file" << imagePath
               << "does not exist.";
      emit decodingFinished(false);
      return "";
    }
...
QImage QZXing::encodeData(const QString &data,
                          ...
                          const bool border, const bool transparent) {
  return encodeData(data, QZXingEncoderConfig(encoderFormat, encoderImageSize,
                                              errorCorrectionLevel, border,
                                              transparent));
}
    ...
    case EncoderFormat_QR_CODE: {
      QSharedPointer<qrcode::QRCode> barcode = qrcode::Encoder::encode(
          ...
          encoderConfig.errorCorrectionLevel == EncodeErrorCorrectionLevel_H
              ...
              : (encoderConfig.errorCorrectionLevel ==
      ...
      const int width =
          int(bytesRef->getWidth()) + (encoderConfig.border ? 2 : 0);
      const int height =
          int(bytesRef->getHeight()) + (encoderConfig.border ? 2 : 0);
